...
<script lang="ts">
  ...
  async function loadComments(): Promise<void> {
    ...
    error = null;
    try {
      comments = await api.comments.list(questionnaireId, {
        anchorType,
        anchorId,
      }) as Comment[];
    } catch (e) {
  ...
  async function createComment(body: string, parentId?: string): Promise<void> {
    try {
      await api.comments.create(questionnaireId, {
  ...
  async function resolveComment(commentId: string, resolved: boolean): Promise<void> {
    try {
      await api.comments.update(questionnaireId, commentId, { resolved });
      await loadComments();
    } catch (e) {
  ...
  async function deleteComment(commentId: string): Promise<void> {
    try {
      await api.comments.delete(questionnaireId, commentId);
      await loadComments();
    } catch (e) {
