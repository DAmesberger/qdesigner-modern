<script lang="ts">
  ...
  function isReactionExperiment(question: Question): boolean {
    return question.type === 'reaction-experiment';
  }
  ...
  let reactionExperimentConfig = $derived(
    isReactionExperiment(question) ? normalizeReactionExperimentConfig(question) : null
  );
  let reactionExperimentSummary = $derived(
    reactionExperimentConfig ? summarizeReactionExperiment(reactionExperimentConfig) : null
  );
...
>
  {#if reactionExperimentConfig && reactionExperimentSummary}
    <div class="rounded-[1.25rem] border border-border/70 bg-[linear-gradient(135deg,hsl(var(--card)),hsl(var(--muted)/0.45))] p-5 shadow-[var(--shadow-sm)]">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
            <FlaskConical class="h-4 w-4" />
            Reaction Lab
          </div>
          <h3 class="mt-3 text-xl font-semibold text-foreground">
            {reactionExperimentConfig.metadata.prompt}
          </h3>
          <p class="mt-2 max-w-3xl text-sm text-muted-foreground">
            {reactionExperimentConfig.metadata.description ||
              'Dedicated timeline-based reaction study with stage rendering, assets, and seeded randomization.'}
          </p>
        </div>
        ...
        <div class="flex items-center gap-2">
          <button
            type="button"
            class="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-4 py-2 text-sm font-medium text-primary transition hover:bg-primary/15"
            onclick={(event) => {
      ...
      <div class="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
        <div class="rounded-2xl border border-border/60 bg-background/80 p-4">
          <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div>
              <p class="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Template</p>
              <p class="mt-1 text-sm font-semibold text-foreground">
                {reactionExperimentConfig.metadata.template}
              </p>
            </div>
            <div>
              <p class="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Structure</p>
              <p class="mt-1 text-sm font-semibold text-foreground">
                {reactionExperimentSummary.blocks} blocks, {reactionExperimentSummary.trials} trials
              </p>
            </div>
            <div>
              <p class="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Stage</p>
              <p class="mt-1 text-sm font-semibold text-foreground">
                {reactionExperimentConfig.stage.width}×{reactionExperimentConfig.stage.height}
              </p>
            </div>
            <div>
              <p class="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Render</p>
              <p class="mt-1 text-sm font-semibold text-foreground">
                {reactionExperimentConfig.stage.targetFPS} FPS
              </p>
            </div>
          </div>

          <div class="mt-4 flex flex-wrap gap-2">
            {#each reactionExperimentSummary.stimulusKinds as kind}
              <span class="rounded-full border border-border/60 bg-muted/60 px-3 py-1 text-xs font-medium text-foreground">
                {kind}
              </span>
            {/each}
            {#if reactionExperimentSummary.stimulusKinds.length === 0}
              <span class="rounded-full border border-dashed border-border/60 px-3 py-1 text-xs text-muted-foreground">
                No stimuli configured yet
              </span>
            {/if}
          </div>
        </div>
        ...
        <div class="grid gap-3">
          <div class="rounded-2xl border border-border/60 bg-background/80 p-4">
            ...
            <p class="mt-2 text-sm text-foreground">
              {reactionExperimentConfig.randomization.conditionStrategy} conditions
            </p>
            <p class="mt-1 text-xs text-muted-foreground">
              Block order: {reactionExperimentConfig.randomization.randomizeBlockOrder ? 'randomized' : 'fixed'}
              {' · '}
              Trial order: {reactionExperimentConfig.randomization.randomizeTrialOrder ? 'randomized' : 'fixed'}
            </p>
          </div>

          <div class="rounded-2xl border border-border/60 bg-background/80 p-4">
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Assets</p>
            <p class="mt-2 text-sm text-foreground">
              {reactionExperimentSummary.assets} linked asset{reactionExperimentSummary.assets !== 1 ? 's' : ''}
            </p>
            <p class="mt-1 text-xs text-muted-foreground">
              Media is managed inside Reaction Lab’s asset bin.
            </p>
          </div>
        </div>
      </div>
    </div>
  {:else if mode === 'edit' && isEditingPrompt}
    <div
