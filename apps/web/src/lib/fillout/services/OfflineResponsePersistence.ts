...
export class OfflineResponsePersistence {
  ...
  static async saveVariable(sessionId: string, name: string, value: unknown): Promise<void> {
    ...
    // Read our own write before another answer-boundary save can replace the
    // composite key. Verification outside this transaction can otherwise read
    // a newer legitimate version and falsely report storage corruption.
    const written = await db.transaction('rw', db.filloutVariables, async () => {
      await db.filloutVariables.put(record);
      return db.filloutVariables.get([sessionId, name]);
    });
    await this.assertWritten(
      written,
      checksum,
  ...
  static async getSessionEvents(sessionId: string): Promise<FilloutEvent[]> {
    return db.filloutEvents.where('sessionId').equals(sessionId).toArray();
  }
  ...
  static async getSessionVariables(sessionId: string): Promise<FilloutVariable[]> {
    const rows = await db.filloutVariables.where('sessionId').equals(sessionId).toArray();
  ...
  static async markResponsesSynced(ids: number[]): Promise<void> {
    await db.filloutResponses.where('id').anyOf(ids).modify({ synced: 1 });
  }
  ...
  static async markEventsSynced(ids: number[]): Promise<void> {
    await db.filloutEvents.where('id').anyOf(ids).modify({ synced: 1 });
  }
  ...
  static async markVariablesSynced(
    ...
    if (accepted === undefined) {
      await db.filloutVariables.where('sessionId').equals(sessionId).modify({ synced: 1 });
      return;
    }
