...
describe('saveVariable', () => {
  it('verifies concurrent versions without mistaking a later write for corruption', async () => {
    await Promise.all(
      Array.from({ length: 12 }, (_, index) =>
        OfflineResponsePersistence.saveVariable('concurrent', 'score', index)
      )
    );
    const rows = await OfflineResponsePersistence.getSessionVariables('concurrent');
