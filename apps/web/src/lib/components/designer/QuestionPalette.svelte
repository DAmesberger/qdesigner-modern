<script lang="ts">
  ...
  async function initializePalette() {
    ...
    try {
      ...
      console.error('Failed to initialize module palette:', error);
      loadError = error instanceof Error ? error.message : 'Failed to load question types';
      modules = [];
    } finally {
    ...
    {:else if loadError}
      <div class="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
        ...
        <button
          ...
          onclick={() => void initializePalette()}
        >
          Retry
        </button>
      </div>
        ...
        >
          <div class="flex items-start space-x-3">
            <span class="text-muted-foreground">
              ...
              {:else if module.type === 'reaction-experiment'}<FlaskConical class="w-6 h-6" />
