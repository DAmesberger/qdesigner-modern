<script lang="ts">
  ...
  let lastLoadedOrganizationId = $state('');
  ...
  const questionTypeLabels: Record<string, string> = {
    ...
    'reaction-experiment': 'Reaction Lab',
    'date-time': 'Date/Time',
  ...
  $effect(() => {
    ...
    if (orgId !== lastLoadedOrganizationId) {
      lastLoadedOrganizationId = orgId;
      void loadTemplates();
    }
  });
  ...
  async function loadTemplates() {
    ...
    try {
      ...
      const result = await withTimeout(
        api.templates.list(organizationId, params),
        TEMPLATE_LOAD_TIMEOUT_MS,
        'Timed out while loading templates'
      );
