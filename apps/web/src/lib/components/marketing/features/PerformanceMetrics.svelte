  ...
  const metrics = [
    {
      ...
      decimals: 1,
      progress: 84,
      description: 'Median interface latency',
    ...
    },
    {
      ...
      decimals: 0,
      progress: 100,
      description: 'Smooth WebGL rendering',
    ...
    },
    {
      ...
      decimals: 1,
      progress: 99.9,
      description: 'Validated capture precision',
...
<section id="performance-metrics" class="py-24 bg-muted/30 relative overflow-hidden">
  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative">
    ...
    <div class="text-center mb-8 sm:mb-12 lg:mb-16">
      <div
        class="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-primary/10 backdrop-blur-sm border border-primary/20 rounded-full text-xs sm:text-sm font-medium text-primary mb-3 sm:mb-4"
      >
      ...
      <h2 class="text-2xl sm:text-3xl lg:text-4xl font-bold mb-3 sm:mb-4">
        Built for
        <span
          class="bg-gradient-to-r from-primary to-primary-foreground bg-clip-text text-transparent"
        >
          extreme performance
        </span>
      </h2>
      <p class="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto px-4">
        Every millisecond counts in research. Our platform is optimized for speed and accuracy at
        every level.
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
      {#each metrics as metric, i}
        <div
          class="bg-card border border-border rounded-xl p-6 relative overflow-hidden group hover:shadow-lg transition-all"
        >
          <!-- Background Gradient -->
          <div
            class="absolute inset-0 bg-gradient-to-br {metric.color} opacity-5 group-hover:opacity-10 transition-opacity"
          ></div>
          ...
          <div class="relative mb-4">
            <svg class="w-32 h-32 mx-auto transform -rotate-90">
              ...
              <circle
                ...
                stroke-dashoffset={`${2 * Math.PI * 56 * (1 - (metric.progress ?? 0) / 100)}`}
              />
              <!-- Gradient Definition -->
              <defs>
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="text-center">
                <span class="text-3xl font-bold text-foreground">
                  {formatMetricValue(metricValues[i] ?? 0, metric.decimals)}
                </span>
    ...
    <div class="mt-16 bg-card border border-border rounded-xl p-8 max-w-4xl mx-auto">
      ...
      <div class="grid md:grid-cols-3 gap-6 text-center">
        <div>
          <div
            class="inline-flex items-center justify-center w-12 h-12 bg-primary/10 rounded-full mb-3"
          >
        ...
        <div>
          ...
          <p class="text-sm text-muted-foreground">
            Sub-millisecond accuracy using performance.now()
          </p>
        </div>
        <div>
          <div
