...
export const dataQualityEntries: HelpEntry[] = [
	...
	},
	{
		...
		description:
			...
			'The design principle: missing stimulus data fails closed and refuses to run, but *degraded timing* is measurable, so it is stamped and continues — never guessed at, never hidden.',
		category: 'dataQuality',
		tags: ['timing', 'validity', 'invalidated', 'provenance', 'visibility', 'cross-origin', 'isolation', 'reaction'],
		related: ['dataQuality.checks', 'analytics.sessionDetail', 'reaction.timingSpec', 'variables.serverVariables']
	}
];
