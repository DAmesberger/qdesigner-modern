<script lang="ts">
  ...
  const normalizedConfig = $derived(normalizeReactionQuestionConfig(question));
  const promptText = $derived(
    typeof question?.config?.prompt === 'string' && question.config.prompt.trim().length > 0
  ...
  const validKeys = $derived(normalizedConfig.response.validKeys);
  const showFeedback = $derived(normalizedConfig.feedback !== false);
