<script lang="ts">
  ...
  const storedConfig = $derived(
    buildModuleRuntimeConfig(question) as unknown as MediaResponseConfig
  );
  function updateConfig(updates: Partial<MediaResponseConfig>) {
  ...
  const hasVideo = $derived(
    config.recordingMode === 'video-audio' || config.recordingMode === 'video-only'
  );
  const hasAudio = $derived(
    config.recordingMode === 'audio' || config.recordingMode === 'video-audio'
  );
</script>
...
<div class="designer-panel">
  ...
  <div class="form-group">
    ...
    <Select
      ...
      onchange={(e) =>
        updateConfig({
          recordingMode: e.currentTarget.value as NonNullable<MediaResponseConfig['recordingMode']>,
        })}
    >
    ...
    <p class="help-text">
      {#if config.recordingMode === 'audio'}
        Participant records audio using their microphone.
      {:else if config.recordingMode === 'video-audio'}
  ...
  <div class="form-group">
    ...
    <Select
      ...
      onchange={(e) => updateConfig({ maxDuration: Number(e.currentTarget.value) })}
    >
  ...
  <div class="form-group">
    ...
    <Select
      ...
      onchange={(e) => updateConfig({ maxFileSize: Number(e.currentTarget.value) })}
    >
      {#each fileSizePresets as preset}
        <option value={preset.value}>{preset.label}</option>
      {/each}
    </Select>
    <p class="help-text">Current: {formatFileSize(config.maxFileSize || 0)}</p>
  </div>
    ...
    <div class="section">
      ...
      <div class="form-group">
        ...
        <Select
          ...
          onchange={(e) =>
            updateConfig({
              audioQuality: e.currentTarget.value as NonNullable<
                MediaResponseConfig['audioQuality']
              >,
            })}
        >
        ...
        <p class="help-text">Higher quality results in larger file sizes.</p>
      </div>
    </div>
  {/if}
    ...
    <div class="section">
      ...
      <div class="form-group">
        ...
        <Select
          ...
          onchange={(e) =>
            updateConfig({
              videoQuality: e.currentTarget.value as NonNullable<
                MediaResponseConfig['videoQuality']
              >,
            })}
        >
        ...
        <p class="help-text">Higher quality requires more bandwidth and storage.</p>
      </div>
    </div>
  {/if}
  ...
  <div class="section">
    ...
    <div class="form-group">
      <label class="checkbox-label">
        <input
          type="checkbox"
          checked={config.allowRerecord}
          onchange={(e) => updateConfig({ allowRerecord: e.currentTarget.checked })}
          class="checkbox"
        />
        <span>Allow re-recording</span>
      </label>
      <p class="help-text">When enabled, participants can discard their recording and try again.</p>
    </div>
    ...
    <div class="form-group">
      ...
      <Select
        ...
        onchange={(e) => updateConfig({ countdown: Number(e.currentTarget.value) })}
      >
      ...
      <div class="preview-content">
        <div class="preview-stats">
          ...
          <div class="stat">
            ...
            <span class="stat-value">{formatDuration(config.maxDuration || 120)}</span>
          </div>
          <div class="stat">
            <span class="stat-label">Max file size:</span>
            <span class="stat-value">{formatFileSize(config.maxFileSize || 0)}</span>
          </div>
          {#if hasAudio}
            <div class="stat">
              <span class="stat-label">Audio quality:</span>
              <span class="stat-value">{getAudioQualityLabel(config.audioQuality || 'medium')}</span
              >
            </div>
          {/if}
          {#if hasVideo}
            <div class="stat">
              ...
              <span class="stat-value">{getVideoQualityLabel(config.videoQuality || 'medium')}</span
              >
            </div>
          {/if}
          <div class="stat">
            ...
            <span class="stat-value">
              {[
                config.allowRerecord && 'Re-record',
                config.countdown && config.countdown > 0 && `${config.countdown}s countdown`,
              ]
                .filter(Boolean)
