...
export interface RenderContext {
  ...
  stimulusTime: number; // Time since content onset
...
/**
 * Base interface for all question content renderers
 * This is what actually displays questionnaire content in WebGL
 */
export interface IQuestionRenderer {
   ...
   */
  prepare(gl: WebGL2RenderingContext, config: RendererConfig): void;
  ...
  /**
   * Get the onset time (when content became visible)
   */
 ...
 */
export abstract class BaseRenderer implements IQuestionRenderer {
  ...
  constructor(id: string, type: string, config: RendererConfig) {
    this.id = id;
    this.type = type;
    this.config = config;
  }
  ...
  public abstract prepare(gl: WebGL2RenderingContext, config: RendererConfig): void;
  ...
  public render(gl: WebGL2RenderingContext, context: RenderContext): void {
    ...
    this.renderContent(gl, renderContext);
  }
  
  protected abstract renderContent(gl: WebGL2RenderingContext, context: RenderContext): void;
