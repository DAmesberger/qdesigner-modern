...
async fn main() {
    ...
    let app = api::router(state.clone())
        ...
        // Outside CORS so that a preflight response — generated by the CORS
        // layer without ever reaching a handler — is decorated too. `test_app`
        // in tests/common mirrors this, the same way it mirrors csrf.
        .layer(axum::middleware::from_fn_with_state(
            state.clone(),
            security_headers,
        ))
