...
#[derive(Debug, Clone)]
struct VariableDefinitionMetadata {
    declared_type: String,
    source_kind: String,
}

#[derive(Debug)]
struct NormalizedVariableValue {
    raw_value: Value,
    value_type: String,
    source_kind: String,
    numeric_value: Option<f64>,
...
fn normalize_variable_name(name: &str) -> Result<String, ApiError> {
...
fn normalize_variable_source_kind(source: Option<&str>) -> String {
    source
        .map(str::trim)
        .filter(|raw| !raw.is_empty())
        .map(|raw| {
...
fn json_value_to_timestamp(value: &Value) -> Option<chrono::DateTime<chrono::Utc>> {
    match value {
...
async fn persist_session_variable(
    ...
    .bind(session_id)
    .bind(context.questionnaire_id)
    .bind(context.questionnaire_version_major)
    .bind(context.questionnaire_version_minor)
    .bind(context.questionnaire_version_patch)
    .bind(variable_name)
    .bind(normalized.value_type)
    .bind(normalized.source_kind)
    .bind(normalized.numeric_value)
    .bind(normalized.text_value)
