...
async fn rls_context_set_and_reset() {
    ...
    // Verify it's set
    let val: String = sqlx::query_scalar("SELECT current_setting('app.user_id', true)")
        .fetch_one(&pool)
        .await
    ...
    let result = sqlx::query_scalar::<_, Option<uuid::Uuid>>("SELECT public.current_app_user_id()")
        .fetch_one(&pool)
        .await;
...
async fn project_member_must_be_org_member() {
    ...
    let user_a = create_test_user(
        &pool,
        &format!("rbac-test-a-{}@example.com", uuid::Uuid::new_v4()),
    )
    .await
    .expect("create user a");
    let user_b = create_test_user(
        &pool,
        &format!("rbac-test-b-{}@example.com", uuid::Uuid::new_v4()),
    )
    .await
    ...
    let org = create_test_org(
        &pool,
        &format!("RBAC Test Org {}", uuid::Uuid::new_v4()),
        user_a,
    )
    .await
    .expect("create org");
    ...
    let project = create_test_project(
        &pool,
        org,
        &format!("Test Proj {}", uuid::Uuid::new_v4()),
        user_a,
    )
    .await
