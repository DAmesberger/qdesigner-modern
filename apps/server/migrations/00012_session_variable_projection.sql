...
CREATE TABLE IF NOT EXISTS questionnaire_variable_definitions (
...
CREATE INDEX IF NOT EXISTS idx_questionnaire_variable_definitions_lookup
    ON questionnaire_variable_definitions(questionnaire_id, version_major, version_minor, version_patch, variable_name);

CREATE TABLE IF NOT EXISTS session_variable_index (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_session_variable_index_numeric
    ON session_variable_index(questionnaire_id, questionnaire_version_major, questionnaire_version_minor, questionnaire_version_patch, variable_name, numeric_value)
    WHERE numeric_value IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_session_variable_index_text
    ON session_variable_index(questionnaire_id, questionnaire_version_major, questionnaire_version_minor, questionnaire_version_patch, variable_name, text_value)
    WHERE text_value IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_session_variable_index_boolean
    ON session_variable_index(questionnaire_id, questionnaire_version_major, questionnaire_version_minor, questionnaire_version_patch, variable_name, boolean_value)
    WHERE boolean_value IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_session_variable_index_timestamp
    ON session_variable_index(questionnaire_id, questionnaire_version_major, questionnaire_version_minor, questionnaire_version_patch, variable_name, timestamp_value)
    WHERE timestamp_value IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_session_variable_index_lookup
    ON session_variable_index(session_id, variable_name);
