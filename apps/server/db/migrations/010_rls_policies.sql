...
-- Always returns false — super_admin is managed at the application layer,
-- not through DB roles. The Rust backend connects as table owner and
-- bypasses RLS, so this function is only evaluated for direct DB access.
