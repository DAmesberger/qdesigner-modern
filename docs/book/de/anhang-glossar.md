...
Eine studienweite Haltung gegenueber verschlechterten Timing-Bedingungen, die in den Fragebogen-Einstellungen gewaehlt wird (ADR 0027). Unter `record` (der Standard) stempelt die Laufzeit die Timing-Provenienz und warnt die Forschenden, stoppt den Teilnehmer aber niemals. Unter `enforce` -- fuer timing-kritische Studien -- greifen zwei Schutzmechanismen: Kann der Browser oder Server keine Cross-Origin-Isolation bereitstellen, verweigert die Studie den Start (ein eigener Bildschirm weist die Teilnehmenden ab, bevor irgendwelche Daten erhoben werden); und ein Fokusverlust mitten im Trial bricht diesen Trial ab, pausiert die Aufgabe auf einer "Zurueck zur Studie"-Einblendung, bis die Teilnehmenden zurueckkehren, und wiederholt den Trial am Blockende (hoechstens drei Wiederholungen pro Block, darueber hinaus wird der Trial als invalidiert-und-verloren aufgezeichnet). Beide Modi werden ausgeliefert. Siehe Kapitel 11.
