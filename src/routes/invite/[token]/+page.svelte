    ...
    {:else if invitation}
      <Card>
        <div class="text-center mb-6">
          ...
          <h2 class="text-2xl font-bold text-foreground">You're Invited!</h2>
        </div>

        <div class="space-y-4">
          <div class="text-center">
            <p class="text-lg text-muted-foreground">
              {invitation.invitedBy?.fullName || invitation.invitedBy?.email} has invited you to join
            </p>
          ...
          {:else if !isAuthenticated}
            ...
            <div class="flex gap-3">
              <Button
                variant="primary"
                class="flex-1"
                onclick={() => invitation && goto(`/login?redirect=/invite/${invitation.token}`)}
              >
                Sign In
              </Button>
              <Button
