...
<div 
  class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-gray-200 dark:border-gray-700 transition-all duration-200 {onClick ? 'cursor-pointer hover:shadow-lg hover:border-gray-300 dark:hover:border-gray-600' : ''}"
  on:click={onClick}
  role={onClick ? 'button' : undefined}
  tabindex={onClick ? 0 : undefined}
  on:keydown={(e) => { if (onClick && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onClick(); } }}
>
  <div class="p-5">
      ...
      <div class="ml-5 w-0 flex-1">
        <dl>
          ...
          <dd class="flex items-baseline">
            ...
            {#if trend && !loading}
              <div class="ml-2 flex items-baseline text-sm font-semibold {getTrendColor(trend.direction)}">
  ...
  {#if onClick}
    ...
    <div class="bg-gray-50 dark:bg-gray-700/50 px-5 py-3">
      <div class="text-sm">
        <span class="text-gray-600 dark:text-gray-400 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
          View details
        </span>
