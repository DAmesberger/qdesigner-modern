...
<div class="presence-indicator flex items-center">
  ...
  <div class="flex -space-x-2">
    {#each visibleUsers as user (user.id)}
      <div
        class="relative {sizeClasses} rounded-full overflow-hidden border-2 border-white {statusClasses[user.status]}"
      ...
      >
        {#if user.avatar}
          ...
          />
        {:else}
          <div class="w-full h-full {getUserColorClass(user.id)} text-white flex items-center justify-center font-medium">
        ...
        <div class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded">
          ...
          <div class="relative w-6 h-6 rounded-full overflow-hidden">
            {#if user.avatar}
              ...
              />
            {:else}
              <div class="w-full h-full {getUserColorClass(user.id)} text-white flex items-center justify-center text-xs font-medium">
                {getUserInitials(user.name)}
              </div>
            {/if}
            
            <!-- Status dot -->
            <div class="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-white
                        {user.status === 'online' ? 'bg-green-500' : 
                          user.status === 'away' ? 'bg-yellow-500' : 'bg-gray-400'}">
            </div>
          </div>
