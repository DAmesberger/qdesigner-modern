<script lang="ts">
  ...
  $effect(() => {
    if (question.answerType) {
      question.answerType = getAnswerType(question.config.responseType.type);
    }
  });
    ...
    {#each randomizedOptions as option (option.id)}
      ...
      >
        <label 
          ...
          class:disabled
          style:border-color={option.color && (question.config.responseType.type === 'single' 
            ? value === option.value 
            : value?.includes(option.value)) ? option.color : undefined}
        >
          <input
            ...
            checked={question.config.responseType.type === 'single' 
              ? value === option.value 
              : value?.includes(option.value)}
            on:change={() => question.config.responseType.type === 'single' 
              ? handleSingleChoice(option) 
              : handleMultipleChoice(option)}
