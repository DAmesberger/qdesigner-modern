<script lang="ts">
  ...
  const colors = $derived(config.colors || ['#000000', '#FF0000', '#00FF00', '#0000FF', '#FFFF00', '#FF00FF']);
  ...
  function saveDrawingData() {
    ...
    if (config.analysis?.trackTiming && strokeData.length > 0) {
      ...
      analysisData.drawingTime = Math.max(...strokeData.flatMap(s => s.points.map(p => p.time)));
    ...
    if (config.analysis?.measurePressure && strokeData.length > 0) {
      ...
      analysisData.pressureVariance = Math.sqrt(
        pressures.reduce((sum, p) => sum + Math.pow(p - analysisData.averagePressure, 2), 0) / pressures.length
      );
    }
    
    value = {
      imageData,
      strokes: config.analysis?.trackTiming || config.analysis?.measurePressure ? strokeData : undefined,
  ...
  <div class="drawing-container">
    <div class="toolbar">
        ...
        <div class="tool-group">
          ...
          <div class="shapes">
            <button
              ...
              on:click={() => currentShape = 'rectangle'}
            >
              ▭
            </button>
            <button
            ...
            <button
              ...
              on:click={() => currentShape = 'triangle'}
            >
              △
            </button>
          </div>
      ...
      <div class="tool-group actions">
        <button class="action-button" on:click={undo} disabled={historyStep <= 0}>
          ↶ Undo
        </button>
        <button class="action-button" on:click={redo} disabled={historyStep >= drawingHistory.length - 1}>
          ↷ Redo
        </button>
        <button class="action-button clear" on:click={clearCanvas}>
          🗑️ Clear
        </button>
      </div>
    </div>
...
<style>
  ...
  .tool-button:hover, .shape-button:hover {
    border-color: #d1d5db;
  }
