<script lang="ts">
  ...
  interface Props extends InstructionProps {
    instruction: InstructionConfig & { config: TextDisplayConfig };
    onContinue?: () => void;
  }
  ...
  $effect(() => {
    if (
      mode === 'runtime' &&
      instruction.config.autoAdvance?.enabled &&
      instruction.config.autoAdvance.delay > 0
    ) {
...
<BaseInstruction {instruction} {mode} {onContinue} {onInteraction}>
  ...
  {#if mode === 'runtime' && instruction.config.autoAdvance?.enabled}
    <div class="auto-advance-indicator">
      This screen will advance automatically in {Math.round(
        instruction.config.autoAdvance.delay / 1000
      )} seconds
    </div>
  {/if}
</BaseInstruction>
