<script lang="ts">
  ...
  $effect(() => {
    ...
    if (
      instruction.media &&
      instruction.media.length > 0 &&
      (!instruction.display?.media || instruction.display.media.length === 0)
    ) {
  ...
  function syncOrphanedMediaReferences() {
    ...
    const currentRefIds = new Set(currentMedia.map((m: MediaConfig) => m.refId));
    const contentRefIds = matches.map((m: RegExpMatchArray) => m[1]);
    ...
    if (orphanedRefIds.length > 0) {
      console.log(
        '[TextInstructionDesigner] Found orphaned media references, removing them:',
        orphanedRefIds
      );
      ...
      orphanedRefIds.forEach((refId) => {
        ...
        cleanedContent = cleanedContent.replace(
          pattern,
          '[Broken media reference - please re-insert]'
        );
      });
  ...
  function handleMediaSelect(event: { media: MediaConfig[]; markdown: string }) {
    ...
    if (firstMedia && firstMedia.mediaId) {
      const mediaId = firstMedia.mediaId;
      mediaService
        .getSignedUrl(mediaId)
        .then((url) => {
          mediaUrls = { ...mediaUrls, [mediaId]: url };
  ...
  <div class="form-group">
    ...
    {#if instruction.display?.media && instruction.display.media.length > 0}
      <div class="media-info">
        {instruction.display.media.length} media item{instruction.display.media.length !== 1
          ? 's'
          : ''} attached
      </div>
    {/if}
