<script lang="ts">
  ...
  const formulaFunctions = [
    ...
    { name: 'DAYS_BETWEEN', signature: 'DAYS_BETWEEN(date1, date2)', description: 'Days between dates' },
  ...
  $: variableSnippets = variables.map(v => ({
    label: v.name,
    kind: 'Variable',
    insertText: v.name,
  ...
  function handleChange(event: CustomEvent) {
    formula = event.detail.value;
    dispatch('change', { value: formula });
  ...
  function handleEditorReady(event: CustomEvent) {
    const { editor, monaco } = event.detail;
    ...
    monaco.languages.registerCompletionItemProvider('formula', {
      provideCompletionItems: (model: Monaco.editor.ITextModel, position: Monaco.Position) => {
        ...
        const suggestions = [
          ...functionSnippets.map(s => ({ ...s, range })),
          ...variableSnippets.map(s => ({ ...s, range }))
        ];
