<script lang="ts">
  ...
  function generateFlowNodes(): Node[] {
    ...
    const usedVariables = Array.from(extractVariablesFromFlows(flowControls));
    usedVariables.forEach((varName, varIndex) => {
      ...
      if (variable) {
        flowNodes.push({
          ...
          position: { x: 100 + (varIndex as number) * 150, y: 50 },
          sourcePosition: Position.Bottom,
        });
      }
    });
  ...
  function generateFlowEdges(nodes: Node[]): Edge[] {
    ...
    if (questionnaire.pages.length > 0 && questionnaire.pages[0]) {
      flowEdges.push({
        id: 'start-page1',
        source: 'start',
    ...
    questionnaire.pages.forEach((page) => {
      if (page.blocks && page.blocks.length > 0) {
        ...
        if (firstBlock) {
          flowEdges.push({
            id: `page-${page.id}-block-${firstBlock.id}`,
            source: `page-${page.id}`,
            target: `block-${firstBlock.id}`,
            type: 'smoothstep',
            animated: true,
          });
        }
        ...
        for (let i = 0; i < page.blocks.length - 1; i++) {
          ...
          if (currentBlock && nextBlock) {
            flowEdges.push({
              id: `block-${currentBlock.id}-${nextBlock.id}`,
              source: `block-${currentBlock.id}`,
              target: `block-${nextBlock.id}`,
              type: 'smoothstep',
            });
          }
        }
        ...
        if (flow.target) {
          ...
          if (sourceId) {
            flowEdges.push({
              id: `flow-${flow.id}`,
              source: sourceId,
              target: flow.target.startsWith('page')
                ? `page-${flow.target}`
                : `question-${flow.target}`,
              type: 'conditional',
              data: {
  ...
  function findSourceForFlow(flow: FlowControl): string | null {
    ...
    if (nodes.length > 0) {
      return nodes[nodes.length - 2]?.id ?? null;
    }
  ...
  const SvelteFlowAny = SvelteFlow as any;
</script>

<div class="flow-control-editor h-full {theme.components.container.card}">
  <SvelteFlowAny
    bind:nodes
    bind:edges
    {nodeTypes}
    ...
    on:connect={(e: any) => handleConnect(e.detail)}
  >
