<script lang="ts">
  ...
  let flowControls: FlowControl[] = [];
  let showFlowEditor = false;
  let editingFlow: FlowControl | null = null;
  let showAddFlow = false;
  ...
  designerStore.subscribe(state => {
    flowControls = state.questionnaire.flow;
  });
  ...
  $: availableTargets = (() => {
    ...
    // Add pages
    $designerStore.questionnaire.pages.forEach((page, index) => {
      targets.push({
        id: page.id,
    ...
    // Add questions
    $designerStore.questionnaire.questions.forEach(question => {
      const pageName = $designerStore.questionnaire.pages.find(p => 
  ...
  function handleUpdateFlow(flowId: string, updates: Partial<FlowControl>) {
    ...
    if (index !== -1) {
      ...
      updatedFlows[index] = { ...updatedFlows[index], ...updates };
...
<div class="{theme.components.container.card} p-4">
  ...
  {#if flowControls.length === 0}
    <div class="text-center py-8">
      ...
      <p class="{theme.typography.caption} {theme.semantic.textTertiary} mt-2">
            ...
            <div class="flex-1">
              ...
              <div class="space-y-1">
                <div class="flex items-center gap-2">
                  <span class="{theme.typography.caption} {theme.semantic.textSecondary}">Condition:</span>
                  <code class="text-xs bg-gray-100 px-2 py-1 rounded">{flow.condition}</code>
                </div>
                
                {#if flow.target}
                  <div class="flex items-center gap-2">
                    <span class="{theme.typography.caption} {theme.semantic.textSecondary}">Target:</span>
                ...
                {#if flow.iterations}
                  <div class="flex items-center gap-2">
                    <span class="{theme.typography.caption} {theme.semantic.textSecondary}">Iterations:</span>
...
{#if showFlowEditor}
  <div class="fixed inset-0 z-50 overflow-hidden">
    <div class="fixed inset-0 bg-black bg-opacity-50" on:click={() => showFlowEditor = false}></div>
    
    <div class="fixed inset-4 {theme.components.container.card} flex flex-col">
