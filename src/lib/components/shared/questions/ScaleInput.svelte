<script lang="ts">
  ...
  $: steps = config.step ? Math.floor(range / config.step) + 1 : range + 1;
  $: buttons = Array.from({ length: steps }, (_, i) => 
    config.min + (i * (config.step || 1))
  );
  
  $: percentage = ((internalValue - config.min) / range) * 100;
</script>
