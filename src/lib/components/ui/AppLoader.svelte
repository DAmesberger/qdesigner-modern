<script lang="ts">
  ...
  let stages: LoadingStage[] = [
    { id: 'service-worker', name: 'Installing offline capabilities', weight: 10, status: 'pending' },
  ...
  onMount(() => {
    ...
    const unsubscribe = serviceWorkerProgress.subscribe(state => {
      if (state.type === 'loading' && state.stage === 'caching') {
        ...
        updateStage('resources', 'loading', state.resource ? `Caching ${state.resource}` : 'Caching resources...');
      }
    });
  ...
  <div class="loader-content">
    ...
    <div class="stages">
      {#each stages as stage}
        <div class="stage" class:complete={stage.status === 'complete'} class:loading={stage.status === 'loading'}>
