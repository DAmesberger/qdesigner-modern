...
<div 
  ...
  style={Object.entries(customStyles).map(([k, v]) => v ? `${k}: ${v}` : '').filter(Boolean).join('; ')}
...
>
  ...
  <!-- Media Content -->
  {#if question.media}
    <div class="question-media media-{question.media.position || 'above'}">
      {#if question.media.type === 'image'}
        <img 
          src={question.media.source} 
          alt={question.media.alt || ''} 
          class="media-content size-{question.media.size || 'medium'}"
        />
      ...
      {#if question.media.caption}
        <p class="media-caption">{question.media.caption}</p>
      {/if}
    </div>
  {/if}
